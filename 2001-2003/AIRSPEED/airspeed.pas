...
const
  ncadr=100;{1/с,количество кадров}
  temp=10000 div ncadr;
  m=500;{кол-во точек образующих поверхность Земли}
  n=1800;{кол-во точек образующих аэропорт}
  s=1500;{кол-во точек образующих небо}
  d:longint=500;{фокусное расстояние линз наблюдателя}
  minh=700;{мм,минимальная высота}
  ver=4;{вероятность засвечивания звезды}
  {--------------}
  aspeed=10;{мм/с^2,изменение ускорения}
  airspeed=195000;{мм/с,минимальная скорость при полёте}
  maxairspeed=1000000000;{мм/с,максимальная скорость}
  dangle=pi/5000;{рад/с}
  {--------------}
  speedz:extended=aspeed/ncadr;
  maxangleh=1e18;{рад,максимальный угол взлёта}
...
begin
  ...
  outtextxy(getmaxx-50,getmaxy-50,scadr);}
  outtextxy(20,20,'Скорость:'+sp+' км/ч');
  outtextxy(500,20,'Высота:'+sh+' м');
  outtextxy(380,20,'Угол:'+ssh+#248);
  setcolor(12);
...
m2:gd:=vga;
  ...
  if f then
   begin
     ...
     setcolor(12);
     delay(50);
     outtextxy(300,220,#7'Упс!!'#7);
     delay(50);
     outtextxy(300,240,'Нажмите любую клавишу...');
     readkey;
     cleardevice;
     goto m1;
   end;
